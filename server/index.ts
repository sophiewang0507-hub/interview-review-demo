...
function buildPrompts(params: { role: string; question: string; answer: string; ragContext: string }) {
  const { role, question, answer, ragContext } = params
...
async function prepareAnalyze(req: express.Request) {
  if (!client) {
    return { ok: false as const, status: 500, error: '缺少环境变量 DASHSCOPE_API_KEY（阿里百炼）' }
  }
  ...
  if (!role || !question || !answer) {
    return { ok: false as const, status: 400, error: 'role/question/answer 不能为空' }
  }

  const examples = await loadExamples()
  const { hits, scored } = retrieveTopK(examples, `${role}\n${question}\n${answer}`, 3)
  const ragContext = buildRagContext(scored)
  const prompts = buildPrompts({ role, question, answer, ragContext })
  return { ok: true as const, role, question, answer, hits, prompts }
}

app.post('/api/analyze', async (req, res) => {
  try {
    const prep = await prepareAnalyze(req)
    if (!prep.ok) {
    ...
    const completion = await client!.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: prep.prompts.system },
        { role: 'user', content: prep.prompts.user },
      ],
    ...
    if (prep.hits.length) (data as any).retrieval = prep.hits
    res.json(data)
  } catch (err: any) {
...
app.post('/api/analyze-stream', async (req, res) => {
  ...
  try {
    const prep = await prepareAnalyze(req)
    if (!prep.ok) {
      send('error', { message: prep.error })
      res.end()
      return
    }

    // 先把 retrieval 发给前端（便于展示“参考示例”）
    send('meta', { retrieval: prep.hits })
    ...
    for await (const chunk of stream as any) {
      ...
      const piece = (delta?.content as string) || ''
      if (!piece) continue
      acc += piece
    ...
    // eslint-disable-next-line no-console
    console.error(err)
    send('error', { message: err?.message || '百炼调用失败' })
    res.end()
  }
})
